...
CommandPlayerStop::CommandPlayerStop( SIncomingCommandServices * _services )
    : ICommandExternal(_services)
{

}

bool CommandPlayerStop::exec(){
    ...
    if( userHasPermission(m_userId, (SIncomingCommandServices *)m_services, errMsg) ){
        ...
        if( player ){
            ...
            Json::Value rootRecord;
            rootRecord[ "cmd_name" ] = "player_stop";
