...
CommandPlayerReverse::CommandPlayerReverse( SIncomingCommandServices * _services )
    : ICommandExternal(_services)
{

}

bool CommandPlayerReverse::exec(){
    ...
    if( userHasPermission(m_userId, (SIncomingCommandServices *)m_services, errMsg) ){
        ...
        if( player ){
            player->switchReverseMode( reverse );

            Json::Value rootRecord;
            rootRecord[ "cmd_name" ] = "player_reverse";
